...
/**
 * Usage: new FileLogger("/home/lvuser/logs/"); Todo: better comments.
 * 
 * @author Jeremy
 *
 */
public class FileLogger {
  ...
  public FileLogger(String logDir) {
    ...
    System.setOut(new PrintStream(new FileWriter(logDir + "out/" + FMS + "_" + alliance + "_"
        + today.toString() + ".txt", out)));
    System.setErr(new PrintStream(new FileWriter(logDir + "err/" + FMS + "_" + alliance + "_"
        + today.toString() + ".txt", err)));
  }
  ...
  private class FileWriter extends OutputStream {
    ...
    public FileWriter(String filename, PrintStream original) {
      ...
      try {
        ...
        // If we cannot write to the file (eg out of space, incorrect
        // permissions, etc)
        // Print to the existing error stream, and don't attempt further
        // writes to the file
        e.printStackTrace();
        fileout = null;
      }
    ...
    public void write(int b) throws IOException {
      ...
      if (fileout != null) {
        ...
        // This will help ensure that we keep as much data as pssible,
        // even if the robot is suddenly powered off
        if (b == '\n') {
          fileout.flush();
        }
      }
