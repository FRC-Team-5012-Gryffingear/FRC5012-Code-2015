	...
	public void run() {
		...
		switch(state) {
			...
			case States.CLOSED_LOOP:	// Open loop state. send input to output
				epc.setEnabled(true);
				if(epc.isUnder()) state = States.MOVING_UP;
				else if(!epc.isUnder()) state = States.MOVING_DN;
				break;
			case States.MOVING_UP:	// Closed loop moving up. Higher gains = moar power
				epc.setEnabled(true);
				epc.setGains(1.0);	// Todo: move to constants.
				if(epc.isNearTarget()) state = States.HOLDING;
				break;
			case States.MOVING_DN:	// Closed loop moving down, lower gains = slower to fall
				epc.setEnabled(true);
				epc.setGains(0.5);
				if(epc.isNearTarget()) state = States.HOLDING;
				break;
			case States.HOLDING:	// Closed loop holding position, super high gains = super holding power
				epc.setEnabled(true);
				epc.setGains(2.0);
				// Exit holding state if 
				if((!epc.isNearTarget()) && epc.isUnder()) state = States.MOVING_UP;
				if((!epc.isNearTarget()) && (!epc.isUnder())) state = States.MOVING_DN;
				break;
		}
